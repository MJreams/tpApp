...
class StudentRegisterModel extends Abs
{
    //	设置当前模型对应的完整数据表名称
    ...
    static public function check($name,$num)
    {
        $res = "ok";
        //验证用户名是否已经存在
        $Name = array('STU_Name' =>$name);
        $Student = self::get($Name);
        if(!is_null($Student))
            //如果存在则返回错误
            $res = "用户名已经存在";
        //验证手机号是否存在
        $map = array('STU_Phone' => $num);
    ...
    {
        $map = array('STU_Phone' => $phone);
        $Student = self::get($map);
        if($Student['STU_Vercode'] == $vercode)
            return true;
        return false;
        //return $Student;
    }
